...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
  ...
  if (!empty($titlu) && !empty($descriere) && $pret_pe_noapte > 0 && $id_hotel > 0) {
    $stmt = $conn->prepare("UPDATE oferte SET titlu=?, descriere=?, pret_pe_noapte=?, id_hotel=? WHERE id=?");
    $stmt->bind_param("ssdii", $titlu, $descriere, $pret_pe_noapte, $id_hotel, $id);
    $stmt->execute();
  }
}
...
if (!empty($search)) {
  $stmt = $conn->prepare("SELECT * FROM oferte WHERE titlu LIKE ?");
  $searchParam = "%$search%";
  $stmt->bind_param("s", $searchParam);
  $stmt->execute();
  $result = $stmt->get_result();
} else {
  $result = $conn->query("SELECT * FROM oferte");
}
?>
<!DOCTYPE html>
<html lang="ro">
...
<body>
  ...
  <div class="edit-container">
    <h2>Modifică Oferte</h2>

    <div class="search-bar">
      <form method="GET">
        <input type="text" name="search" placeholder="Caută ofertă..." value="<?= htmlspecialchars($search) ?>">
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            <form method="POST">
              ...
              <td><input type="text" name="titlu" value="<?= htmlspecialchars($row['titlu']) ?>" required></td>
              <td><textarea name="descriere" required><?= htmlspecialchars($row['descriere']) ?></textarea></td>
              <td><input type="number" step="0.01" name="pret_pe_noapte"
                  value="<?= htmlspecialchars($row['pret_pe_noapte']) ?>" required></td>
              <td><input type="number" name="id_hotel" value="<?= htmlspecialchars($row['id_hotel']) ?>" required></td>
