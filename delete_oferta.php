...
if (isset($_GET['id'])) {
  $id = intval($_GET['id']);
  $stmt = $conn->prepare("DELETE FROM oferte WHERE id = ?");
  $stmt->bind_param("i", $id);
  $stmt->execute();

  header('Location: delete_oferta.php');
  exit();
}

// Selectăm toate ofertele
      ...
      <tbody>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= htmlspecialchars(number_format($row['pret_pe_noapte'], 2)) ?> €</td>
            <td><?= htmlspecialchars($row['id_hotel']) ?></td>
            <td><a class="delete-button" href="delete_oferta.php?id=<?= $row['id'] ?>"
                onclick="return confirm('Sigur vrei să ștergi această ofertă?');">Șterge</a></td>
          </tr>
        <?php endwhile; ?>
      </tbody>
    </table>
