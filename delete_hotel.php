...
if (isset($_GET['id'])) {
  $id = intval($_GET['id']);
  $stmt = $conn->prepare("DELETE FROM hoteluri WHERE id = ?");
      ...
      <tbody>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($row['stele']) ?></td>
            <td><?= htmlspecialchars($row['id_destinatie']) ?></td>
            <td><img src="imagini/<?= htmlspecialchars($row['imagine']) ?>" alt="Imagine Hotel"></td>
            <td><a class="delete-button" href="delete_hotel.php?id=<?= $row['id'] ?>"
                onclick="return confirm('Sigur vrei să ștergi acest hotel?');">Șterge</a></td>
          </tr>
        <?php endwhile; ?>
      </tbody>
    </table>
