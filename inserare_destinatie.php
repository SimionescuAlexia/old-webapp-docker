  ...
  if (isset($_FILES['imagine']) && $_FILES['imagine']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($fileExtension, $allowedExtensions)) {
      ...
      if (move_uploaded_file($fileTmpPath, $destPath)) {
        $stmt = $conn->prepare("INSERT INTO destinatii (tara, oras, imagine) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $tara, $oras, $newFileName);
        $stmt->execute();

        header('Location: destinatii.php');
        exit();
      } else {
        $error = "Eroare la încarcarea imaginii.";
      }
    } else {
...
<body>
  ...
  <div class="form-container">
    <h2 style="margin-bottom: 20px; color: #00796b;">Adaugă Destinație</h2>
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <input type="text" name="tara" id="tara" required>
